...
process_i386_opcodes (FILE *table)
{
  ...
  filename = "heptane-opc.tbl";
  fp = fopen (filename, "r");

  if (fp == NULL)
    fail (_("can't find heptane-opc.tbl for reading, errno = %s\n"),
	  xstrerror (errno));
  ...
  fprintf (table, "\n/* heptane opcode table.  */\n\n");
  fprintf (table, "const insn_template heptane_optab[] =\n{\n");
...
process_i386_registers (FILE *table)
{
  ...
  filename = "heptane-reg.tbl";
  fp = fopen (filename, "r");
  if (fp == NULL)
    fail (_("can't find heptane-reg.tbl for reading, errno = %s\n"),
	  xstrerror (errno));

  fprintf (table, "\n/* heptane register table.  */\n\n");
  fprintf (table, "const reg_entry heptane_regtab[] =\n{\n");
  ...
  fprintf (table, "\nconst unsigned int heptane_regtab_size = ARRAY_SIZE (hetane_regtab);\n");
}
...
{
  unsigned int i;
  FILE *fp = fopen ("heptane-init.h", "w");
  char *init;

  if (fp == NULL)
    fail (_("can't create heptane-init.h, errno = %s\n"),
	  xstrerror (errno));
...
main (int argc, char **argv)
{
  ...
  if (c)
    fail (_("%d unused bits in heptane_cpu_flags.\n"), c);
#endif
  ...
  if (c)
    fail (_("%d unused bits in heptane_operand_type.\n"), c);
#endif
  ...
  table = fopen ("heptane-tbl.h", "w");
  if (table == NULL)
    fail (_("can't create heptane-tbl.h, errno = %s\n"),
	  xstrerror (errno));
